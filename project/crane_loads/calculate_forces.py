...
class Calculate_Crane_Forces():
    ...
    def __init__(self):
        ...
        Ht_out1 = self.calc_Ht(L, a, v_out1["SQr_MAX"], v_out1["SQr_MIN"],
                               v_out1["Qr_min"], mf, mw, v5, nr)
        
        Ht_out4 = self.calc_Ht(L, a, v_out4["SQr_MAX"], v_out4["SQr_MIN"],
                               v_out4["Qr_min"], mf, mw, v5, nr)

        # Υπολογισμός των οριζόντιων φορτίων Hs για τους συνδυασμούς 1 και 5
        # Τα φορτία αποθηκεύονται σε dictionary με όνομα Ηs_out$
        Hs_out = self.calc_Hs(rolling_type, L, nr, e1, e2, m,
                              v_out5["SQr_MAX"], 
                              v_out5["SQr_MIN"], 
                              a_rad = 0.015)
        
        # Υπολογισμός του φορτίου κόπωσης
        fat_out = self.calc_fatigue(Gcr, Gtr, Qr_nom, L,
                                    e_min, lfat_s, lfat_t, vfat)
        
        self.print_greek(v_out1, v_out2, v_out3, v_out4, v_out5, 
                         Ht_out1, Ht_out4, Hs_out, 
                         fat_out, vfat, lfat_s, lfat_t,
                         a_rad, rolling_type, fatigue_class)

    def print_greek(self, v_out1, v_out2, v_out3, v_out4, v_out5,
                    Ht_out1, Ht_out4, Hs_out, 
                    fat_out, vfat, lfat_s, lfat_t,
                    a_rad, rolling_type, fatigue_class):
        ...
        s = self.choose_file(s, Hs_out, a_rad, rolling_type)
        ...
        s = self.print_fatigue(s, fat_out, vfat, lfat_s, lfat_t, fatigue_class)
        s = self.write_to_file(s, "fatigue.tex")
        
    def choose_file(self, s, Hs_out, a_rad, rolling_type):
        raw_file = rolling_type + "_raw.tex"
        tex_file = rolling_type + ".tex"
    ...
    def calc_Hs(self, rolling_type, L, n, e1, e2, m,
                SQr_max, SQr_min, a_rad = 0.015):
        """
        ...
        if rolling_type.endswith('FF'):
            h = (m * ksi1 * ksi2 * L**2 + e1**2 + e2**2)/(e1 + e2)
        elif rolling_type.endswith('FM'):
    ...
    def print_fatigue(self, s, out, vfat, lfat_s, lfat_t, fatigue_class):
        """
        ...
        s = s.replace("fatigue_class--", "{0}".format(fatigue_class), 1)
    ...
    def print_table(self, s, v_out1, v_out2, v_out3, v_out4, v_out5,
                    Ht_out1, Ht_out4, Hs_out):
        """
